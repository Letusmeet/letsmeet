...
router.post('/comment',(req,res)=>{
    ...
    Card.findById(card).then(mycard=>{
       ...
       mycard.update({$push:{comments:mycomment}}).then(done=>{
        ...
        res.json({err,message:"Some error occured in adding comment, please try again!"});
    })
})
...
router.post('/changestatus',(req,res)=>{
    ...
    Card.findById(card).then(mycard=>{
       mycard.update({status:status}).then(done=>{
        ...
        res.json({err,message:"Some error occured while changing status, please try again!"});
    })
})
...
router.post('/deleteboard',(req,res)=>{
    ...
    Board.findById(board).then(found=>{
        found.cards.forEach(item=>{
            Card.findByIdAndRemove(item._id).then(card=>{
                console.log(card);
            }).catch(err=>{
        ...
        found.remove().then(done=>{
            ...
            res.json({err,message:"Some error occured while deleting board, please try again!"});
        })
...
router.post('/deletecard',(req,res)=>{
    ...
    Card.findById(card).then(mycard=>{
        ...
        res.json({err,message:"Some error occured while deleting card, please try again!"});
    })
})
